...
import style from "./productPreviev.module.css";
...
export const ProductPreviev = (linkId) => {
   ...
   const toggleFavorite = () => {
    ...
    const existingProductIndex = cardData.findIndex(item => item.projectId === product.id);
    if (existingProductIndex !== -1) {
        // Продукт уже в избранном, удаляем его
        cardData = cardData.filter(item => item.projectId !== product.id);
        setFavorite(false);
    } else {
...
useEffect(() => {
    ...
    if (storedData) {
        ...
        const isFavorite = cardData.some(item => item.projectId === product.id);
